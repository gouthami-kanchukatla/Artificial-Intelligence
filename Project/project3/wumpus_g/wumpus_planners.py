...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        distances=[]
        distances = [  manhattan_distance_with_heading(node.state,goal)  for goal in self.goals  ]  #calculating the distances of each goal node from the start node
        heuristic = min(distances) #finding the nearest goal node
        return(heuristic)
    ...
    def actions(self, state):
        ...
        """
        dir_1 = ['Forward','TurnLeft','TurnRight']
        dir_2 = ['TurnLeft','TurnRight']
        
        if state[2] == 0 and (state[0],state[1]+1) in self.allowed: return(dir_1) #IF FACING NORTH
            
        if state[2] == 1 and (state[0]-1,state[1]) in self.allowed:  return(dir_1)      #IF FACING WEST
            
        if state[2] == 2 and (state[0],state[1]-1) in self.allowed:  return(dir_1)      #IF FACING SOUTH
        
        if state[2] == 3 and (state[0]+1,state[1]) in self.allowed:  return(dir_1)       #IF FACING EAST
    ...
    def result(self, state, action):
        ...
        if action == 'Forward':
            if state[2] == 0: return(state[0],state[1] + 1,state[2])  #MOVE FORWARD IN Y DIRECTION(X,Y=Y+1)
            if state[2] == 1: return(state[0]-1,state[1],state[2])#IF WEST
                   #MOVE BACKWARD IN X DIRECTION (X=X-1,Y)
            if state[2] == 2: return(state[0],state[1]-1,state[2]) #IF SOUTH
                   #MOVE BACKWARD IN Y DIRECTION (X,Y=Y-1)
            if state[2] == 3:  return(state[0]+1,state[1],state[2]) #IF EAST
                 #MOVE FORWARD IN X DIRECTION(X=X+1,Y)

        if action == 'TurnRight':
            if state[2] == 0:  return(state[0],state[1],3)#if NORTH
                  #RESULT FACING EAST
            if state[2] == 1:  return(state[0],state[1],0) #IF WEST
                  #RESULT FACING NORTH
            if state[2] == 2:  return(state[0],state[1],1)  #IF SOUTH
                  #RESULT FACING WEST
            if state[2] == 3:  return(state[0],state[1],2) #IF EAST
                   #RESULT FACING SOUTH


        if  action == 'TurnLeft':
            if state[2] == 0:  return(state[0],state[1],1)#if NORTH
                  #RESULT FACING WEST
            if state[2] == 1:  return(state[0],state[1],2) #IF WEST
                #RESULT FACING SOUTH
            if state[2] == 2:  return (state[0],state[1],3)#IF SOUTH
                 #RESULT FACING EAST
            if state[2] == 3:  return(state[0],state[1],0) #IF EAST
                 #RESULT FACING NORTH
        pass
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        spots_shot = []
        wump_locs = self.goals
        explorer_locs = self.allowed

        for loc in wump_locs:
            for ex_loc in explorer_locs:
                if (loc[0] == ex_loc[0]) or (loc[1] == ex_loc[1]): spots_shot.append(ex_loc)

        spots_shot_distance = [  manhattan_distance_with_heading( node.state, goal) for goal in spots_shot ]

        heuristic = min(spots_shot_distance)
        return heuristic
    ...
    def actions(self, state):
        ...
        if  (state[0],state[1]+1)  and  state[2] == 0 in self.allowed:  return(list_1)
        if (state[0]-1,state[1]) and  state[2] == 1 in self.allowed:    return(list_1)
        if  (state[0],state[1]-1)  and  state[2] == 2  in self.allowed:  return(list_1)
        if  (state[0]+1,state[1]) and state[2] == 3 in self.allowed:   return(list_1)
    ...
    def result(self, state, action):
        ...
        if action == 'Forward':
            if state[2] == 0:  return(  state[0], state[1] + 1, state[2] )  #IF NORTH
                 #MOVE FORWARD IN Y DIRECTION(X,Y=Y+1)
            if state[2] == 1:  return(  state[0]-1, state[1], state[2] ) #IF WEST
                  #MOVE BACKWARD IN X DIRECTION (X=X-1,Y)
            if state[2] == 2:  return( state[0], state[1]-1, state[2])  #IF SOUTH
                  #MOVE BACKWARD IN Y DIRECTION (X,Y=Y-1)
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        wumpus_locs = self.goals
        
        if state in self.goals:  return False   #RETURN FALSE IF YOUR CURRENT STATE MATCHES WITH ONE OF THE GOAL STAT

        for loc in wumpus_locs:  #CHECH ALL THE GOAL NODES ONE AT A TIME

            if loc[0] == state[0]:   #IF BOTH X CORDINATES MATCHES THEN 
                if ( state[2] == 0 and ( loc[1] > state[1] ) ):  return True#IF Y CORDINATE OF GOAL NODE IS GREATER THAN CURRENT STATE AND THE DIRECTION IS NORTH
                       #RETURN TRUE
                if (  state[2] == 2 and ( loc[1] < state[1] ) ): return True  #IF Y CORDINATE OF GOAL NODE IS LESS THAN CURRENT STATE AND THE DIRECTION IS SOUTH
                    #RETURN TRUE
            
            if  loc[1] == state[1]: #IF BOTH Y CORDINATES MATCHES THEN 
                if (  state[2] == 1 and  (loc[0] < state[0] ) ): return True #IF X CORDINATE OF GOAL NODE IS LESS THAN CURRENT STATE AND THE DIRECTION IS WEST
                     #RETURN TRUE
                if (  state[2] == 3  and  (loc[0] > state[0])  ): return True  #IF X CORDINATE OF GOAL NODE IS GREATER THAN CURRENT STATE AND THE DIRECTION IS EAST
                    #RETURN TRUE
